...
//******************************
//** Start Laser Export Code
//*****************************

// ---------- Laser export controls ----------
laser_kerf = 0.20;                   // total kerf width (mm). Tweak per material/machine.
function k_outer() = +laser_kerf/2;  // outer edges grow out
function k_inner() = -laser_kerf/2;  // holes/shapes shrink in

// extra XY bleed for Orca cutters to avoid slivers at tabs/edges
cutter_bleed = 0.12;   // 0.08–0.2 mm works well


// 2D: base + TAB features only (no slots)
module tile2d_outer_only() {
  pairs = [
  ...
  union() {
    base_hex(edge_len);
    for (p = pairs)
      if (p[2]) place_feature_on_edge(edge_len, p[0], feat_dist, /*tab=*/true,  p[3]);
  }
}

// 2D: SLOT features only (as positive shapes we will subtract)
module tile2d_slots_only() {
  pairs = [
    [0, 3, pair_0_3, pair_0_3_type],
    [1, 4, pair_1_4, pair_1_4_type],
    [2, 5, pair_2_5, pair_2_5_type]
  ...
  union() {
    for (p = pairs)
      if (p[2]) place_feature_on_edge(edge_len, p[1], edge_len - feat_dist, /*tab=*/false, p[3]);
  }
}

// 2D: single "unit" tile outline with the repeating Orca through-cuts
module tile2d_unit_raw() {
  difference() {
    tile2d_outer_only();                         // outer boundary (tabs included)
    tile2d_slots_only();                         // slots removed
    repeating_orca_cutters_local(-1,1,-1,1);    // Orca motif removed
  }
}

// Use this when exporting SVG for cutting
//module tile2d_laser_unit() {
//  difference() {
//    // outer edges grow by +kerf/2
//    offset(delta = k_outer()) tile2d_outer_only();
//
//    // inner cuts (slots + Orca) shrink by -kerf/2
//    offset(delta = k_inner()) union() {
//      tile2d_slots_only();
...
module tile2d_laser_unit() {
  difference() {
    offset(delta = k_outer()) tile2d_outer_only();
    union() {
      offset(delta = k_inner())                 tile2d_slots_only();
      offset(delta = k_inner() + cutter_bleed)  repeating_orca_cutters_local(-1,1,-1,1);
    }
  }
}
...
module tile3d_through_orca_unit() {
  difference() {
    ...
    translate([0,0,-0.05])
      linear_extrude(thickness + 0.1)
        offset(delta = +fit_clear + cutter_bleed)
          repeating_orca_cutters_local(-1, 1, -1, 1);
  }
}
...
//module tile3d_through_orca_unit() {
//  difference() {
//    tile3d();  // your tile with features (tabs/slots)
//    // subtract Orca motif with extra bleed so openings are fully cut
//    translate([0,0,-0.05])
//      linear_extrude(thickness + 0.1)
//        offset(delta = inlay_clear + cutter_bleed)
//          repeating_orca_cutters_local(-1, 1, -1, 1);
//  }
//}
...
/*
How to export (SVG):
    1. Set show_grid = false.
    2. Replace your render body with just:

    tile2d_laser_unit();

    3. Preview (F5) and Export → Export as SVG.
    
    In the dialog: check “Stroke Outline”, uncheck “Fill Geometry”, stroke width 0.01–0.05 mm.
    If your laser CAM already applies kerf, set laser_kerf = 0 here.
*/
